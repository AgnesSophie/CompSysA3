...
{
    //to_hash takes the length of password + length of salt as argument
    char to_hash[strlen(password) + strlen(salt)];

    //copies strlen(password) characters from password to to_hash.
    memcpy(to_hash, password, strlen(password));
    //copies strlen(salt) characters from salt to to_hash+strlen(password).
    memcpy(to_hash+strlen(password), salt, strlen(salt));
    //uses get_data_sha to get a hash of the specified data : to_hash.
    //the hash itself is placed in the variable 'hash'.
    ...
    {
        printf("this is hash[%c]", to_hash[i]);
    }
    printf("\n");
}
...
int main(int argc, char **argv)
{
    ...
    get_signature(password, user_salt, signature);   
